...
O script cria `.claude/tasks/AG-XXXXX/branch-state.json` com todos os campos
...
## Passo 0d — Briefing do Desenvolvedor

**Antes de qualquer investigacao autonoma**, solicite que o desenvolvedor apresente
sua compreensao da tarefa:

> "Antes de comecar, descreva com suas proprias palavras:
> 1. **O que esta tarefa faz** e qual o impacto esperado no sistema?
> 2. **Qual o fluxo** que voce imagina para a implementacao?
> 3. **Quais arquivos, units ou rotinas** voce ja identificou como ponto de entrada?
>
> Isso garante que voce entende a tarefa antes de avancarmos."

**Aguarde a resposta. Nao prossiga para a Etapa 1 sem ela.**

- Se a resposta demonstrar entendimento do fluxo → use os pontos citados como seeds adicionais para as queries RAG.
- Se a resposta for vaga ou demonstrar que o desenvolvedor nao leu a AG → sinalize:
  > "Recomendo revisar a AG e suas referencias tecnicas antes de continuar. Quando estiver pronto, descreva o fluxo."
...
python "$SCRIPTS/viasuperdev/vault_search.py" --id PROC-XXX

# Por termo livre
python "$SCRIPTS/viasuperdev/vault_search.py" "<regra de negocio>"

# Por secao especifica
python "$SCRIPTS/viasuperdev/vault_search.py" "<termo>" --section regras-negocio
