...
description: >
  ...
  implementacao de AG, analise de regras de negocio IBS/CBS, rateio proporcional,
  notas de credito/debito, reforma tributaria, desenvolvimento Delphi Viasuper,
  Oracle, TFFormProc, TFFormPadraoCds, QueryPegaData, RespOk, ou qualquer
  referencia a rotinas do sistema Viasuper. Tambem aciona quando o usuario pede
  brainstorm tecnico, revisao de diff, mensagem de commit, ou analise de AG.
---
...
de uma AG: analise tecnica com RAG semantico, implementacao Delphi com worktree,
validacao e abertura de PR.
...
- **Conectores:** TMS com xData (padrao); `DmConexao.QueryPegaData` para queries.
...
| `python vault_search.py --id RN-004` | Busca por ID exato no vault (fallback camada 2) |
| `python vault_search.py "<termo>"` | Busca textual direta no vault |
| `python vault_search.py --impact AG-XXXXX` | Documentos que compartilham artefatos com a AG |
...
O `branch-state.json` fica em `.claude/viasuperdev/AG-XXXXX/branch-state.json`:
```json
{
  "ticket": "AG-XXXXX",
  "fase_atual": 1,
...
| `NAO_ENCONTRADA` | AG nao existe | Lista AGs disponiveis |
...
### Fase 2 — Implementacao, PR e Encerramento

Leia e execute: `.claude/skills/viasuperdev/references/modo-fase2.md`

> O encerramento (mensagem de commit, link do PR, mover AG para `done/`) esta
> coberto nos Passos 4 e 5 do `modo-fase2.md`.
...
python vault_search.py --list   # lista todos os documentos, incluindo PT-XXX
```
