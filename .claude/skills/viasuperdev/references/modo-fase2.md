...
Leia `.claude/tasks/AG-XXXXX/branch-state.json`.
...
```bash
python "$SCRIPTS/viasuperdev/vault_search.py" --id PT-001
python "$SCRIPTS/viasuperdev/vault_search.py" --id PT-002
# repita para cada PT-XXX listado em `04_Padroes_Tecnicos/`
```
 
Para listar todos os padroes tecnicos disponiveis no vault:
```bash
python "$SCRIPTS/viasuperdev/vault_search.py" --list
```
...
> Commit, push e abertura do PR sao responsabilidade do desenvolvedor.
> A skill nunca executa `git commit`, `git push` ou abre PR.
...
Pergunte ao usuario se deseja mover a AG para `done/`. Se confirmado:
```bash
python -c "
